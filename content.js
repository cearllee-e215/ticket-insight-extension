// Content Script - Ticket Insight v2.0
...
(function() {
   ...
   function getPriceColor(idx, total) {
         ...
         var C = [
                 [76,175,80], [129,199,132], [189,189,40],
                 [255,235,59], [255,193,7], [255,152,0],
                 [255,87,34], [244,67,54], [183,28,28]
               ];
         var sl = 1 / (C.length - 1);
   ...
   function processFacetsData(data) {
      ...
      data.facets.forEach(function(facet) {
              ...
              var tp = facet.totalPriceRange ? facet.totalPriceRange[0].min : lp;
              var bk = Math.ceil(lp / 10) * 10;
              if (!buckets[bk]) buckets[bk] = { count: 0, bucket: bk, minTotal: Infinity };
              buckets[bk].count += facet.count;
              buckets[bk].minTotal = Math.min(buckets[bk].minTotal, tp);
              ...
              var isResale = invTypes.indexOf("resale") >= 0;
                                ...
                                facet.offers.forEach(function(oid) {
                                          ...
                                          var offerType = offer.inventoryType || (isPrimary ? "primary" : isResale ? "resale" : "unknown");
                                                             ...
                                                             if (!sections[sec]) {
                                                                         sections[sec] = {
                                                                                       section: sec, count: 0, primaryCount: 0, resaleCount: 0,
                                          ...
                                          var totalP = offer.totalPrice || tp;
                                          ...
                                          sd.maxTotal = Math.max(sd.maxTotal, totalP);
                                                             ...
                                                             sd.listings.push({
                                                                         row: offer.row, seatFrom: offer.seatFrom, seatTo: offer.seatTo,
                                                                         listPrice: listP, totalPrice: totalP, count: facet.count, type: offerType
                                                             });
                                });
      });
      ...
      var total = sortedBk.reduce(function(s,b){ return s + b.count; }, 0);
         ...
         sortedSec.forEach(function(sec) { primary += sec.primaryCount; resale += sec.resaleCount; });

      return {
              priceBuckets: sortedBk, sections: sortedSec,
              totalTickets: total, primaryCount: primary, resaleCount: resale,
   ...
   function shadeSeatMap(p, activeBucketIdx) {
      ...
      // Find section blocks using data-section-name
      var blocks = document.querySelectorAll('[data-component="svg__block"]');
         blocks.forEach(function(block) {
                 ...
                 var color = getPriceHex(bucketIdx, total);
                              ...
                              // Get all seat circles in this section
      ...
      // Also shade any dots using generic selectors as fallback
      var genericBlocks = document.querySelectorAll('[data-section-id], [data-section]');
         genericBlocks.forEach(function(el) {
                 if (el.hasAttribute("data-component") && el.getAttribute("data-component") === "svg__block") return;
                 var secId = el.getAttribute("data-section-name") || el.getAttribute("data-section") || "";
                 if (!secId) {
                           var sid = el.getAttribute("data-section-id") || "";
                           // Try to map section-id like "s_11" back to a name
                   blocks.forEach(function(b) {
                               if (b.getAttribute("data-section-id") === sid) secId = b.getAttribute("data-section-name");
                   });
                 }
                 if (secId && sectionPriceMap[secId] !== undefined) {
                           var bucketIdx = sectionPriceMap[secId];
                           var color = getPriceHex(bucketIdx, total);
                           if (activeBucketIdx !== null && activeBucketIdx !== undefined) {
                                       el.style.fill = bucketIdx === activeBucketIdx ? color : "#e0e0e0";
                                       el.style.opacity = bucketIdx === activeBucketIdx ? "0.85" : "0.2";
                           } else {
                                       el.style.fill = color;
      ...
      console.log("[TicketInsight] Shaded " + blocks.length + " section blocks" +
                        (activeBucketIdx !== null ? " (highlighting bucket " + activeBucketIdx + ")" : ""));
   }
   ...
   function startShadeObserver(p) {
      ...
      // Re-shade periodically since TM dynamically updates the map
      shadeInterval = setInterval(function() {
              if (processedData) {
                        shadeSeatMap(processedData, selectedBucketIdx);
              }
      }, 3000);

      // Also observe DOM changes in the SVG area
      var svgContainer = document.querySelector('svg[viewBox]');
         if (svgContainer) {
                 shadeObserver = new MutationObserver(function() {
                           if (processedData) {
                                       setTimeout(function() { shadeSeatMap(processedData, selectedBucketIdx); }, 500);
                           }
                 });
                 shadeObserver.observe(svgContainer, { childList: true, subtree: true, attributes: true, attributeFilter: ["style", "class"] });
         }
   }
   ...
   function renderSecondaryMarketOverlay() {
      ...
      markets.forEach(function(m) {
              ...
              btn.href = m.url; btn.target = "_blank"; btn.rel = "noopener noreferrer";
   ...
   function renderOverlay(p) {
      ...
      var hdr = document.createElement("div");
         ...
         ic.textContent = "\uD83C\uDFAB"; ic.style.cssText = "font-size:1.3em;margin-right:5px;";
         var tt = document.createElement("span");
         tt.textContent = "Price Points"; tt.style.cssText = "font-weight:bold;font-size:1.15em;color:#333;";
         tw.appendChild(ic); tw.appendChild(tt);

      var smBtn = document.createElement("span");
         smBtn.textContent = "\uD83D\uDD0D"; smBtn.title = "Compare Secondary Markets";
      ...
      var sw = document.createElement("div");
         ...
         p.priceBuckets.forEach(function(bk, i) {
                 ...
                 dot.style.cssText = "width:18px;height:18px;border-radius:50%;flex-shrink:0;background:" + getPriceColor(i, p.priceBuckets.length) +
                 ...
                 countLabel.textContent = "(" + bk.count.toLocaleString() + ")";
                                      ...
                                      r.addEventListener("click", function() {
                                                ...
                                                if (selectedBucketIdx === idx) {
                                                            ...
                                                            dot.style.borderColor = "#1565c0";
                                                            this.style.background = "rgba(21,101,192,0.08)";
                                                            shadeSeatMap(p, idx);
                                                }
                                      });
                 ...
                 r.addEventListener("mouseleave", function() {
                           if (selectedBucketIdx !== parseInt(this.getAttribute("data-bucket-idx")))
                                      ...
                                      r.appendChild(dot); r.appendChild(priceLabel); r.appendChild(countLabel);
                 sw.appendChild(r);
                 swatchRows.push(r);
         });
      ...
      var custWrap = document.createElement("div");
         custWrap.style.cssText = "margin-top:6px;padding-top:5px;border-top:1px solid #e0e0e0;";
      ...
      var shadeBtn = document.createElement("button");
         ...
         shadeBtn.addEventListener("click", function() {
                 selectedBucketIdx = null;
                 swatchRows.forEach(function(sr) { sr.querySelector("[data-swatch]").style.borderColor = "transparent"; sr.style.background = ""; });
                 shadeSeatMap(p, null);
         });
      ...
      custContent.appendChild(shadeBtn); custContent.appendChild(clearBtn);
         ...
         custWrap.appendChild(custToggle); custWrap.appendChild(custContent);
      ...
      console.log("[TicketInsight] Rendered: " + p.priceBuckets.length + " price levels, " + p.sections.length + " sections, " + p.totalTickets + " tickets");
   }
   ...
   console.log("[TicketInsight] Content script loaded v2.0");
})();
